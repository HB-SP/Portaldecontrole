...
| — | `supabase_folgas.sql` | Cria o controle de folgas e presença (6 tabelas `folgas_*`). Traz as próprias policies dentro do arquivo, então a tabela não fica aberta entre ele e o `seguranca.sql` — que também já conhece as seis. Depois de rodar, povoe com `scripts/importar_folgas.mjs --gravar` e `scripts/feriados_folgas.mjs --gravar` |
